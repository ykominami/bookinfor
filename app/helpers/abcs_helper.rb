module AbcsHelper
    ...
    class TotalScorex
        ...
        def initialize(header)
            ...
            Choicex.get_name_values.map{ |name|
    ...
    class Choicex
        ...
        def self.get_choice_id(name)
            @keys_by_name[name]
        end
        def self.get_name_values
            @names.values
        end
        def self.get_name_keys
            @names.keys
        end
    ...
    class Votex
        ...
        def initialize(user_name , vote_result)
            @user_name = user_name
            @vote_id = Votex.get_next_vote_id
